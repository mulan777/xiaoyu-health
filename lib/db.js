...
async function initDatabase(config) {
  ...
  // 2026-09-09 企业微信无感登录: wx_userid 列 + 唯一索引(幂等)
  await ensureColumnExists(database, 'users', 'wx_userid', "wx_userid VARCHAR(64) NULL COMMENT '企业微信userid'");
  const wxIdx = await dbQuery(`SELECT COUNT(*) AS total FROM information_schema.STATISTICS WHERE TABLE_SCHEMA = ? AND TABLE_NAME = 'users' AND INDEX_NAME = 'uk_users_wx_userid'`, [database]);
  if (!Number(wxIdx[0]?.total || 0)) {
    await dbQuery('ALTER TABLE users ADD UNIQUE KEY uk_users_wx_userid (wx_userid)');
  }
