// routes/user.js — 教师端 /user 路由(从 server.js 抽取, 2026-07-27)
// 依赖通过 mountUserRoutes(app, upload, ctx) 注入, 与 venue.js/admin.js 模式一致
...
const { parseWorkbookRows } = require('../lib/excel');

module.exports = function mountUserRoutes(app, upload, ctx) {
  const {
  ...
  app.get('/user', requireRole('user'), asyncHandler(async (req, res) => {
    ...
    // 班级体测趋势 — 按测试日期聚合各项目均分（范围级纵向对比）
    var scopeTrendData = [];
    var trendDateMap = {};
    for (var _ti = 0; _ti < allFitnessRecords.length; _ti++) {
      var item = allFitnessRecords[_ti];
      if (!item.test_date) continue;
      var day = formatDateOnly(item.test_date);
      if (!trendDateMap[day]) {
        trendDateMap[day] = {
          testDate: day,
